
/*

  single quote string

  this kind of string will not be interpolated

  ''' ... '''
  ' ... '

  '
  { something  }: this is not be interpolated
  a.b.c: this is not be interpolated
  \r \n but this will be escaped

  '

  ''' this will not be escaped '''

  s'...'
  s'''...'''

  no interpolated for this kind of escape string
 */
var STRING, UNDENT_TOO_MUCH_IN_STRING, repeat;

STRING = require('../../token-constant').STRING;

repeat = require('../../../utils').repeat;

exports.singleQuoteString = function() {
  var c, c2, cursor, i, indent, lead, line1, lineInfo, lineno, myLineInfo, quote, start, str, text;
  text = this.text, cursor = this.cursor;
  if (text[cursor + 1] === "'") {
    if (text[cursor + 2] === "'") {
      quote = "'''";
      start = cursor;
      cursor += 3;
    } else {
      this.cursor += 2;
      this.setToken(STRING, "''", cursor, this.lineno);
    }
  } else {
    start = cursor;
    cursor++;
    quote = "'";
  }
  lineno = this.lineno, lineInfo = this.lineInfo;
  line1 = lineno;
  indent = lineInfo[lineno].lead;
  str = '';
  i = 0;
  while (c = text[cursor]) {
    if (i++ > 1000) {
      break;
    }
    switch (c) {
      case "'":
        if (quote === c) {
          this.cursor++;
          this.lineno = lineno;
          return this.setToken(STRING, '"' + str + '"', start, line1);
        } else if (text.slice(cursor + 1, +(cursor + 2) + 1 || 9e9) !== "''") {
          cursor++;
          str += '"';
        } else {
          this.cursor += 3;
          this.lineno = lineno;
          return this.setToken(STRING, '"' + str + '"', start, line1);
        }
        break;
      case '"':
        str += '\\"';
        cursor++;
        break;
      case '\\':
        if ((c2 = text[cursor + 1]) === '\n') {
          lineno++;
          myLineInfo = lineInfo[lineno];
          lead = myLineInfo.lead;
          if (lead < indent) {
            this.lexError(UNDENT_TOO_MUCH_IN_STRING);
          }
          str += text.slice(cursor + 2, cursor + 2 + lead);
          cursor += 2 + lead;
        } else if (c2 === '\r') {
          lineno++;
          myLineInfo = lineInfo[lineno];
          lead = myLineInfo.lead;
          if (lead < indent) {
            this.lexError(UNDENT_TOO_MUCH_IN_STRING);
          }
          str += text.slice(cursor + 3, cursor + 3 + lead);
          cursor += 3 + lead;
        } else {
          cursor++;
          str += '\\\\';
        }
        break;
      case '\n':
        lineno++;
        myLineInfo = lineInfo[lineno];
        lead = myLineInfo.lead;
        if (lead < indent) {
          this.lexError(UNDENT_TOO_MUCH_IN_STRING);
        }
        str += text.slice(cursor, cursor + 1 + lead);
        cursor += 1 + lead;
        break;
      case '\r':
        lineno++;
        myLineInfo = lineInfo[lineno];
        lead = myLineInfo.lead;
        if (lead < indent) {
          this.lexError(UNDENT_TOO_MUCH_IN_STRING);
        }
        str += text.slice(cursor, cursor + 2 + lead);
        cursor += 2 + lead;
        break;
      default:
        cursor++;
        str += c;
    }
  }
  return this.lexError('expect ' + quote + ', unexpected end of input in string');
};

exports.UNDENT_TOO_MUCH_IN_STRING = UNDENT_TOO_MUCH_IN_STRING = 'wrong indent. undent too much. all lines should not indent less than the first line of the string';

exports.singleQuoteString.leadingString = "'";
